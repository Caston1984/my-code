#https://github.com/ardaegeunlu/X-armed-Bandits/blob/master/BanditAlgorithm/HOO.py
# -*- coding: utf-8 -*-
import numpy as np
import TreeNode
import random
...
class HOO(object):
    ...
    def selection_of_arm_from_covering(self, h, i):
        ...
        # be further modified.
        selected_arm = (1.0*subset_space.upper + subset_space.lower) / 2.0   #original
        print("selected arm = {0}".format(selected_arm))
        print('a', selected_arm[0])
        self.last_arm = selected_arm
        reward = self.environment_function(selected_arm)
        # Get the reward and item collected
        #reward, collected = MaxLength.reward(self, max_trajectory=50, weight= selected_arm[0])
        return reward
